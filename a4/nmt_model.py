...
class NMT(nn.Module):
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            # hyp_lens = torch.tensor([float(len(hyp) + 1) for hyp in hypotheses], dtype=torch.float, device=self.device)
            # contiuating_hyp_scores = torch.div(hyp_scores.unsqueeze(1).expand_as(log_p_t) + log_p_t, hyp_lens.unsqueeze(1).expand_as(log_p_t)).view(-1)
