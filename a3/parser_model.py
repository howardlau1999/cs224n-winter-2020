...
class ParserModel(nn.Module):
    ...
    def forward(self, w):
        ...
        x = self.embedding_lookup(w) # x is (batch_size, n_features * embed_size)
        h = self.dropout(F.relu(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)) # h is (batch_size, hidden_size)
        logits = torch.matmul(h, self.hidden_to_logits_weight) + self.hidden_to_logits_bias # logits is (batch_size, n_classes)
